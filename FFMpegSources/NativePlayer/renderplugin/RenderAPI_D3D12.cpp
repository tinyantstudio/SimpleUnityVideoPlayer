...
int RenderAPI_D3D12::GetRowPitch(int width, int height, int pixelSize)
{
	return max(AlignPow2(width * pixelSize), 256);
}
...
void RenderAPI_D3D12::ModifyTextureWithInputData(void* textureHandle, int textureWidth, int textureHeight, int rowPitch, void* dataPtr)
{
	ID3D12Resource* resource = (ID3D12Resource*)textureHandle;
	if (resource != NULL && dataPtr != NULL)
	{
		ID3D12Device* device = s_D3D12->GetDevice();

		const UINT64 kDataSize = GetAlignedSize(textureWidth, textureHeight, 1, rowPitch);
		ID3D12Resource* upload = GetUploadResource(kDataSize);
		upload->Unmap(0, NULL);
